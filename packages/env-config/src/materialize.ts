...
export const PR_NUMBER: number = Number(process.env.PR_NUMBER ?? 0);
...
function generateDotenv(runtimeConfigData: RuntimeVariables) {
...
function generateWranglerJsonc(runtimeConfigData: RuntimeVariables) {
  ...
  let extractedEnvVariant: RecordEntry | undefined;
