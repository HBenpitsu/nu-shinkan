...
本書は `188ea27` の追跡ファイルを比較基準として，2026-09-06 にコードを読み合わせた結果を反映しています．実ファイル名は `deploiy-impl-plan.md` です．今回の更新は計画文書のみで，以下の実装・実環境検証は未実施です．確定した振る舞いはデプロイ設計に従い，既存実装，実装予定，方式の検証が残る項目を区別します．
