...
[デプロイ設計](../ADR/deploy-design.md)を実装するための作業，実現方式の検討，検証項目を整理します．[設計草案](../ADR/deploy-design-draft.md)と従来の設計文書にあったコマンド例，実装上の注意，未決定事項を引き継いでいます．用語は[メンタルモデル](../ADR/deploy-model.md)に従います．
...
`connections.bindings` と `connections.urls` から，呼び出し元から接続先に向く辺を作ります．自己ループ・実在しない接続先を除外し，循環があっても停止する探索を実装します．

起点から逆向きに辿って `reviewEntry: true` へ至るすべてのパスに含まれるパッケージを収集します．指定した起点自身を含めて影響先を求める段階と，connection グラフで確認経路を選ぶ段階を区別します．manual-pick でも確認経路の収集は設計に定めた規則に従い，指定パッケージをそのまま最終候補とする処理に置き換えないようにします．
...
| `G`     | 呼び出し元から接続先へ辺を張った connection グラフ                             |
| `G_rev` | `G` のすべての辺を逆向きにしたグラフ                                       |

1. `S` の全ノードから `G_rev` を探索し，到達可能な集合 `F` を求める．
2. `E` の全ノードから `G` を探索し，確認入口から到達できる集合 `R` を求める．
3. `targets = F ∩ R` とする．
...
たとえば `Web（reviewEntry）→ 中継 API → 変更 API` と `バッチ → 変更 API` があり，確認入口からバッチへの経路がない場合，`F` にはバッチも含まれますが `R` には含まれません．結果として，変更 API・中継 API・Web を選びます．
