...
`pnpm sync` は `scripts/sync-local/sync.ts` から `turbo run sync:local` を実行し、全パッケージへの反映に成功してから共有local設定のnullを削除する。各パッケージの `sync:local` はapp-configの `sync-local` CLIを実行する。app-configの単一パッケージ更新処理は共有設定を変更しない。
