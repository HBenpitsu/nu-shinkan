...
workspaceの取得・検証・Worker情報収集は `scripts/workspace/`、計画とタスク実行・結果集計は `scripts/deploy/` が担当する。`.github/` は入力の取得とGitHub固有の制御・通知を担当し、CLIとJSONでリポジトリ処理へ接続する。入口と入出力は [scriptsの説明](../../scripts/README.md#deploy) を参照する。
